...
document.querySelector('input[placeholder="Custom"]').addEventListener('focus',()=>{
    for(let i = 0;i< optionButton.length;i++){
...
document.querySelector('button[type="reset"]').addEventListener('click', () => {
    ...
    numberOfPeople.style.color = "hsl(184, 14%, 56%)";
    for(let i = 0; i < optionButton.length;i++){
        optionButton[i].style.backgroundColor = "hsl(183, 100%, 15%)";
        optionButton[i].style.color = "white";

    }
    document.querySelector('.tip-per-person .calculated-result').innerText = '0.00';
